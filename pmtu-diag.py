...
def measure_reply_mtu(target, total_size, iface, timeout, ident, count):
    """Returns (frag_count, reply_mtu, sent_ok).
    ...
    return (frag_total, reply_mtu, sent_ok)
...
def main():
    ...
    ap.add_argument("target", help="destination IP address")
    ...
    print("  Target: %s%s%s   Iface: %s%s%s   Probes/test: %s%d%s" %
          (C.BLD, target, C.RST, C.BLD, iface or "?", C.RST, C.BLD, count, C.RST))
    ...
    frag_count, reply_mtu, reply_sent_ok = measure_reply_mtu(
        target, reply_probe_size, iface, timeout, ident=0x4000, count=count)

    reply_frag = False
    if not reply_sent_ok:
